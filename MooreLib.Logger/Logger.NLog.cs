...
namespace MooreLib.Logging
{
    ...
    {
        private static readonly Layout ExceptionTextLayout = Layout.FromString("${exception:format=tostring}");
        ...
        private Func<DateTime> _currentDateProvider = () => DateTime.Today;
        ...
        private sealed class PreparedConfiguration
        {
            ...
            public DateTime ActiveFileDate { get; }

            public bool RolloverRequired { get; }

            public bool ForceArchiveOnNextEligibleWrite { get; }
                ...
                bool forceArchiveOnNextEligibleWrite)
            {
                ...
                FileTarget = fileTarget;
                FilePath = filePath;
                ActiveFileDate = activeFileDate;
                RolloverRequired = rolloverRequired;
                ForceArchiveOnNextEligibleWrite = forceArchiveOnNextEligibleWrite;
            }
        }
        ...
        private void ApplyPreparedConfigurationLocked(PreparedConfiguration prepared)
        {
            ...
            {
                if (previous != null && !ReferenceEquals(_logFactory.Configuration, previous))
                {
                    try
                    {
        ...
        private void LogPhysicalLocked(PhysicalEvent logEvent, LogLevel level, bool startsNewPhysicalLine)
        {
            ...
            var fileEligible = _fileTarget != null && (int)level >= (int)_minimumFileLevel;
            if (_forceArchiveOnNextEligibleWrite && fileEligible && _fileTarget != null)
            {
        ...
        {
            if (_fileTarget == null || _fileLogPath == null || (int)upcomingLevel < (int)_minimumFileLevel)
            {
                return;
            }
            ...
            {
                var bySize = _options.ArchivePolicy as FileArchivePolicy.BySize;

                if (bySize != null)
                {
                    _rolloverRequired = _fileLengthProvider(_fileLogPath) >= bySize.MaximumFileSizeBytes;
                }
                else if (_options.ArchivePolicy is FileArchivePolicy.Daily)
                {
                    _rolloverRequired = _currentDateProvider().Date > _activeFileDate;
                }
                else
                {
                    _rolloverRequired = false;
